...
import {
 ...
 class ProxyDevice implements IProxyDevice {
    ...
    getRemoteTime(): boolean {
       if (!this.parser) {
          ...
          return this.parser.getRemoteTime();
       }
    }
 ...
 export class DeviceClass implements IDeviceClass {
    ...
    getClassId() {
       ...
       return '3e0f78de-c19e-11ea-b3de-0242ac130004';
    }
    ...
    if (vid === '16C0' && pid === '0483')
       deviceName = 'Tensy_4_1'; //Due Native port 003E
    ...
    //    deviceName = 'Sparkfun Thing Plus SAMD51';
       ...
       const deviceVersionTimeout = global.setTimeout(() => {
          ...
          const err = new Error(
             `Timed out: device ${deviceName} did not respond to version request.`
          );
          console.warn(err);
          callback(err, null);
       }, kTimeoutms);
